...
把当期成片自动发到 B站，并发表 + 置顶「今日日报」评论。`video:meta` 会一次生成 `data-scheme/video-meta.json` 与 `data-scheme/comments.txt`。

## 命令

```bash
# 从零到发布：video:auto-generate → video:render → 渲封面 → video:meta（标题/标签+评论）→ bili:full
bun run all:bili

# 数据已备好、只想发 B站：video:render → 渲封面 → video:meta（标题/标签+评论）→ bili:full
bun run publish:bili
```

也可拆开单步：`video:meta`（生成标题/标签与时间轴评论）、`comment:generate`（只重生成评论）、`bili:upload`（纯投稿，已带 `--no-comment`，只发视频不发评论/置顶，纯测试可用它发一条试稿，发完记得去创作中心删）、`bili:full`（投稿+评论+置顶 全套）、`bili:comment` / `bili:stick`（单独发评/置顶）。`publish:bili` / `all:bili` 内部都调 `bili:full`。
...
由 `video:meta` 用 LLM 生成，写到 `data-scheme/video-meta.json`——可手改再审。模型会读取完整 `stories`（标题、Tabs、Scenes），以 AI 日报短视频为应用场景理解整期内容，再选 1～2 个主打点；不能只看标题列表，也不能把评测/预览误写成正式发布。
