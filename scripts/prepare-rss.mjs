...
//    workspace — nothing to archive or clear, so let rss build it from scratch.
let date;
try {
...
//    archived — the current data-scheme is stale leftover, clear it so rss starts
...
if (archiveResult.status !== 0) {
  console.error(
    `❌ archive 失败 (exit ${archiveResult.status ?? "null"})，已终止以保护数据，rss 未执行。`,
  );
  process.exit(1);
}

console.log("✅ archive 完成，data-scheme/ 已就绪，rss 将生成全新数据。");
