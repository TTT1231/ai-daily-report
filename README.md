...
>
> `all_proxy` 是可选项：**仅作用于 `rss` 抓取阶段**（RSS 源 + 该阶段 AI 评分）；未配置时这些请求直接连接；配置后上述 rss 阶段请求必须使用该代理，代理无效或不可用时会报错，不会静默改为直连。其他代理环境变量不会被读取。MiniMax TTS、B站 发布等 Node 端请求不走此代理，始终直连。
