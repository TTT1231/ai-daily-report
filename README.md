...
成片固定为 **1920×1080 横屏**。带 `overlayImg` 的 scene 会在整段旁白期间把证据图作为主画面，
字幕固定在下方；Tabs 只为没有图片的 scene 兜底。项目不再维护 9:16 正文版，手机观众按横屏全屏观看。
...
看到好新闻想直接做？把**链接、文本、截图**丢给装了本项目的 AI 助手，说一句：

```
/vision-generate-video <粘贴你的素材>
```

AI 会产出一条强制带证据截图的日报视频。

**这个入口挑模型**：它依赖**多模态能力**——模型得能直接看懂证据截图、亲手画 SVG 图标，才走得通这条快路径。所以如果你的 AI 助手是多模态的（如 Codex、Claude Code 这类），`/vision-generate-video` 就是它的**半自动**出片方式：你负责挑素材，剩下的交给它；纯文字模型用不了这条路，请走方式二的网页勾选。
