...
各模式容量：supplied-source / codex-generate-video 模式下带图证据段最多 5 个、每 Story 总 scene ≤6，无图事实段的数量与排列由 agent 按事实决定（选图规则见 [`supplied-source-mode.md`](./supplied-source-mode.md)）；原生 RSS 与手动模式按各自规则处理。
