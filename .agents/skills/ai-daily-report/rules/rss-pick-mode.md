...
4. 对每条补选新闻生成一个 `DailyStory`：
   ...
   - `scenes`：1-4 个（全局 Schema 上限为 6；RSS 自动模式内部控制在 4），普通新闻 1 个、独立核心事件或独立证据段才增加；证据段多的条目建议在段间或结尾留一个无图 scene 方便读 Tabs（可选）；每个 subtitle 是完整口播句，避免标题党和未经证实扩写。
...
- 如果补选数量很多，优先保持每条 2 个 tab、1-2 个 scene，避免视频过长；证据确实充分的条目可按 RSS 模式内部上限放宽到 4 个 scene。
