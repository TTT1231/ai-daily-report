...
- 正文最多 5 个 topTitle 栏目，不含概览和结语；同类 Story 连续，不能以宽度够用为由增加分类。
- 每个正文 Scene 必须有支撑当前口播的来源图片。同图可跨多个连续 Scene 显式复用，不机械继承不相关图片。
- 核心证据无法取得的 Story 排除并报告原因；全部无法取证时保留现有数据包。不插入无图段帮助读 Tabs。
- 口播时长随证据信息量决定，取消固定的短句/5–8 秒目标。标题核心必须在口播中解释。
- Intro 每类展示一个代表标题与该类条数，完整 Story 目录由底部导航承担。
